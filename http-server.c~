...
//	char phoneNumber[1000];  // variable for phone number received
        ...
        const char *requestURI, FILE *mdbFp, int mdbSock, int clntSock)
{
	...
	//this is where I create the child to send the sms 	
	char *phoneNumber = "9172380164";

    	/*    
	char tempPhoneNumber [1000];
		char *k = strstr(line,"number: ");
	...
	*/
		...
		if (pid < 0) {
		     ...
		     //fprintf(stderr, "Hck4Hmny crashed %s\n",phoneNumber);
		     execl("./Hck4Hmnty.sh", "Hck4Hmnty.sh", phoneNumber, line, (char *)0);
		     die("execl failed");
...
int main(int argc, char *argv[])
{
	...
	//printf("%s", requestLine);
        ...
        if (strcmp(method, "GET") != 0) {
            statusCode = 501; // "This is it. Not Implemented"
            sendStatusLine(clntSock, statusCode);
            goto loop_end;
        }
	...
	if (strstr(requestURI, textRequest) != 0) {
		...
		fprintf(stderr, "\n%s\n\n", textMessage);
				...
				//parse out the text
				 char *s = strstr(requestURI, "From=%2B1");
			 ...
			 printf( "\n%s\n\n", phoneNumber);
			 char keyURI[1000] = "/mdb-lookup?key";
			// char randomFormat[1000] ="number: ";
			// requestURI = strcat(keyURI, randomFormat);	 
			 requestURI = strcat(keyURI, phoneNumber);				
			 requestURI = strcat(phoneNumber, "\n");
			 handleMdbRequest(requestURI, mdbFp, mdbSock, clntSock);
	}
	else {
